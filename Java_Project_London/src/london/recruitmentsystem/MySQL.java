...
public class MySQL {
    ...
    public static String[] getLoggedInCompanyInfo(){
     ...
     public static int[] getJobIDList(String userSearch){
         int numberOfRows = -1;
         try{
             stmt = MySQL.conn.createStatement();
     ...
     public static String[] getJobCompanyList(String userSearch){
         int numberOfRows = -1;
         try{
             stmt = MySQL.conn.createStatement();
             query = "SELECT id FROM Job WHERE name LIKE '%"+ userSearch +"%';";
     ...
     public static String[] getJobCityList(String userSearch){
         int numberOfRows = -1;
         try{
             stmt = MySQL.conn.createStatement();
             query = "SELECT id FROM Job WHERE name LIKE '%"+ userSearch +"%';";
     ...
     public static String[] getJobCountryList(String userSearch){
         int numberOfRows = -1;
         try{
             stmt = MySQL.conn.createStatement();
             query = "SELECT id FROM Job WHERE name LIKE '%"+ userSearch +"%';";
    ...
    public static String[] getJobsDetails(int jobID){
        String[] infos = new String[10];
        try{
            stmt = MySQL.conn.createStatement();
            query = "SELECT street, postCode, city, country FROM Address WHERE id=(SELECT Address FROM Job WHERE id=" + jobID + ");";
            rs = MySQL.stmt.executeQuery(query);
            try{
                if(rs.next()){
                    infos[0] = rs.getString(1);
            ...
            query = "SELECT Name, contractType, partOrFullTime, description, salary FROM Job WHERE id=" + jobID + ";";
            rs = MySQL.stmt.executeQuery(query);
            try{
                if(rs.next()){
                    infos[4] = rs.getString(1);
                    infos[5] = rs.getString(2);
                    infos[7] = rs.getString(4);
		    infos[6] = rs.getString(3);
		    infos[8] = rs.getString(5);
                }
            }
            catch(SQLException e){
                System.out.println("SQLException when calling getString(String query) --> rs.next() & str = rs.getString(1) : " + e.getMessage());
            ...
            query = "SELECT name FROM Company WHERE id=" + jobID + ";";
            rs = MySQL.stmt.executeQuery(query);
            try{
                if(rs.next()){
                    infos[9] = rs.getString(1);
